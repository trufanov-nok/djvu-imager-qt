...
void
display(const GURL &url, char* dumppath)
{
   ...
   if((outfile = fopen(dumppath, "wb" )) == NULL )
  	   
	   G_THROW( ERR_MSG("Output dump txt file was not created.") );

   //fputs((const char*)ns, stdout);

   fputs((const char*)ns, outfile);
...
usage()
{
  DjVuPrintErrorUTF8(
          ...
          "Usage: djvudumpm <iff_filenames> <dump_file_path>\n" );
  exit(1);
}
  ...
  G_TRY
    {
      if (argc<=1)
        usage();
      //for (int i=1; i<argc; i++) // to allow 3-d parameter - the dump file path
	  for (int i=1; i<2; i++)
      {
